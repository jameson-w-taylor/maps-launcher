...
// Lat/Long for pin
const pinLatLong = [47.5951518,-122.3316393];
...
export class HomePage {
  ...
  async openAppleMap() {
    if (await this.canOpenMapApp('apple')) {
      ...
      try {
        const { completed } = await AppLauncher.openUrl({ url: this.generateLaunchURL('apple', 'pin') });
  ...
  async openAppleMapDirections() {
    if (await this.canOpenMapApp('apple')) {
      try {
        const { completed } = await AppLauncher.openUrl({ url: this.generateLaunchURL('apple', 'directions') });
  ...
  async openGoogleMap() {
    ...
    try {
      const { completed } = await AppLauncher.openUrl({ url: this.generateLaunchURL('google', 'pin') });
  ...
  async openGoogleMapDirections () {
    ...
    if (canOpen) {
      try {
        ...
        const { completed } = await AppLauncher.openUrl({ url: this.generateLaunchURL('google', 'directions') });
  ...
  private async canOpenMapApp (targetApp: 'google'|'apple') {
    switch(targetApp) {
      case 'apple': {
        if (!this.isIOS) {
          return false;
  ...
  private generateLaunchURL(targetApp: 'google'|'apple', mapType: 'pin'|'directions') {
    switch(targetApp) {
      ...
      case 'apple': {
        ...
        if (mapType === 'pin') {
          ...
          return `maps://?ll=${pinLatLong.join(',')}&${urlEncodedParams.toString()}`;
        } else {
          ...
          return `maps://?dirflg=t&daddr=${pinLatLong.join(',')}`;
        }
      }
      ...
      case 'google': {
        if (mapType === 'pin') {
          ...
          launchUrl.searchParams.append('query', pinLatLong.join(','));
          return launchUrl.href;
        } else {
          if (this.isIOS) {
            // https://developers.google.com/maps/documentation/urls/ios-urlscheme#directions
            return `comgooglemaps://?directionsmode=driving&daddr=${pinLatLong.join(',')}`;
          }
          // https://developers.google.com/maps/documentation/urls/android-intents
          return `google.navigation:mode=d&q=${pinLatLong.join(',')}`;
        }
      }
    }
  }
